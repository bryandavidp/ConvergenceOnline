...
- **`docs/RENDER_STABILITY_PLAN.md`** — **estabilidad de render / presupuesto de capas del compositor y memoria de GPU** (tareas RS-*): por qué en Android de alta densidad (Xiaomi Mi Pad 7) las fichas **parpadeaban a negro** y las imágenes cargaban a medias (desalojo de backing stores por presión de GPU: el mismo bug que en iOS se ve blanco), y la corrección: sin `filter`/`mix-blend-mode` animados en superficies grandes, `will-change` disciplinado, `decoding="async"`, gobernador consciente del dispositivo. Consultarlo antes de tocar el compositing del tablero (`.board-wrap`), añadir capas full-screen, `will-change`/`mix-blend-mode`/`filter` animados, o imágenes.
