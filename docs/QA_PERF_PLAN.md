...
|---|---|---|---|---|
| QP-1 | **Corrección de bugs confirmados** (§2.1) | Los 6 hallazgos [CONFIRMADO] de la auditoría | 🟡 | Tests de regresión por cada uno; suite verde |
...
- (d) **Gobernador v2 con histéresis y escalones**: medir EMA como hoy pero actuar por niveles — nivel 0 (todo), nivel 1 (EMA>20ms sostenido 2s: ambientales de tablero fuera + `FX.cap` 28), nivel 2 (EMA>26ms: pulsos de tiles fuera + popups sin animación de escala + cap 18), con re-subida solo tras 10s buenos (histéresis). Aplicado con una clase en `<body>` (`perf-1`/`perf-2`) para que sea CSS puro y auditable. Corregir de paso el tope (`cap` no debe superar 50).
- (e) **Auto-sugerencia de `reduced-fx`**: si el nivel 2 se mantiene >30s en una sesión, toast único "¿Activar modo ligero?" con acción directa (respeta la elección para siempre). El dato medido (57 FPS con reduced-fx) garantiza que la opción de verdad resuelve.
- (f) **Presupuesto de confeti por evento en móvil**: los `FX.confetti(70–110)` de récords/hitos crean esa cantidad de nodos DOM de golpe; escalar el argumento por `FX.cap/50` para que en gama baja un récord tire 30 y no 110.
