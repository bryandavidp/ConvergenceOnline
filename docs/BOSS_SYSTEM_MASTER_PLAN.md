...
| **Ronda maestra** (derrotado sin perder vida NI usar potenciador en el encuentro) | Botín + **+1 vida** (respetando tope MAX+1; si al tope: +50 de suministro) + sello ✦ en bestiario | Homenaje directo a Gungeon (P6); reutiliza `_noBoosterSinceBoss` y `_livesLostThisWave` ya existentes |
...
| **BOREAL** — el aliento blanco (`frost`) | I | 2 | Escarcha: congela 3+nivel iconos dispersos | Congela un clúster 2×2 compacto | Rompe hielos pronto (+2 suministro c/u ya existe, B-06) | Descongela todo + 25 de suministro |
| **EL CERRAJERO** — guardián de candados (`lockdown`) | II | 3 (1 blindada) | Cierre: siembra 2-3 candados de 1 golpe | **Roba 1 potenciador del inventario del intento** y lo enjaula en un tile `cage` visible — romper la jaula (adyacencia) lo devuelve | Convergencias adyacentes rompen candados y jaulas | Devuelve exactamente lo enjaulado; si no robó nada, +25 de suministro |
...
### 2026-07-13 — Corrección de post-lanzamiento (v2.6.22→2.6.23) · el jefe siempre aparece + presentación legible + layout responsive

Tres bugs reportados por el propietario tras cerrar el plan, con su causa raíz y fix:

- **BUG-1 · «avisa un jefe (2.º) pero nunca aparece y da SUPERADO + bendición directa» — el sistema VIEJO seguía asomando.** Causa raíz: `Bosses._placeAnchors` gateaba el nº de anclas por `Survival._specialRoom()` (`SPECIAL_CAP` − especiales). En oleadas de jefe avanzadas el tablero llega saturado (hielos/candados acumulados **+** los traps que `newWave` siembra ANTES de `bossEvent`), así que `_specialRoom()`→0 → **0 anclas** → `startEncounter` caía al **fallback del jefe-evento clásico** (`_runBoss` + `_afterBossEvent`): efecto instantáneo viejo, sin banner/cuerpo/card, y a +1.2/1.7s el beat «SUPERADO» y la bendición. Exactamente el síntoma. **Fix:** las anclas son el CUERPO transitorio del jefe (se retiran al resolver) y la única vía de daño → **dejan de gatearse por `SPECIAL_CAP`** (`n = min(def.anchors, f.length)`). El jefe aparece siempre que haya iconos donde anclar; el exceso sobre el cap es temporal y se autocura al resolver, y mientras dura `_specialRoom()`→0 frena los ataques que siembran especiales (no sobrecarga el tablero). El fallback clásico solo queda para el caso patológico `f.length===0` (jamás en juego real) y para `ENCOUNTERS=false`. **«Lo antiguo deja de funcionar» en la práctica.**
- **BUG-2 · presentaciones del jefe demasiado veloces, sin tiempo de reacción.** Causa: `startEncounter` no llamaba a `_lock` (la entrada corría sin retención) y la `boss card` animaba **1.15s** entrada+salida incluidas (≈0.5s legible). **Fix:** `Survival._lock(1200, def.frame)` en la entrada (retención de presentación; el 1.er ataque sigue a ~6s) + `@keyframes bossCard` a **2.6s** con meseta 7%→86% (≈2s legibles). En `reduced-fx` la card sigue sin existir (el banner informa).
- **BUG-3 · el tablero se redimensiona en pantallas de poca altura y la parte inferior se recorta según el navegador.** Dos causas y dos fixes:
  - *(a) Recorte inferior cross-browser:* `#app`/`body` usaban `100vh`/`100dvh`; en móvil `100vh` = viewport GRANDE (barra oculta) → más alto que el área visible → `booster-bar` bajo la barra. **Fix:** var JS `--app-h` = alto realmente visible (`visualViewport.height||innerHeight`, re-medido en `resize`/`orientationchange`/`visualViewport`); `#app`/`body` usan `height: var(--app-h, 100dvh)`.
  - *(b) El tablero encogía/se rompía en pantallas cortas (iPhone SE 375×667, etc.):* la supervivencia usaba `--board-size: clamp(280px, min(96vw, calc(100svh−494px)), 620px)` — la reserva de 494px acoplaba el tablero al ALTO y lo dejaba en 280px (75% del ancho) o menos, y el HUD de alrededor no se compactaba. **Fix (rediseño responsive del layout SV):** (1) rejilla `grid-template-rows: auto minmax(0,1fr) auto` — HUD arriba, TABLERO en la región flexible del medio, boosters `auto` abajo (siempre visibles, `align-self: end`); (2) el tablero se dimensiona **por ANCHO** (`--board-size: min(88vw, 540px)`) pero se acota a la región con `body.mode-surv .board-wrap { height:100%; aspect-ratio:1; max-width/max-height: var(--board-size) }` → SIEMPRE cuadrado y **jamás desborda la región** (imposible solaparse con HUD/boosters, sin reservas mágicas); (3) el cromo del HUD se compactó a alturas fijas (event-dock 22px, score-row 46px, booster 68/56px, wallet-pills 24px, gtop-context gap/padding 4px, surv-bar padding/gap fijos) para que quepa el tablero grande. Resultado: **tablero FIJO POR ANCHO** (88vw, cuadrado) idéntico entre alturas en todos los teléfonos modernos; en ventanas muy cortas (desktop bajo, SE1 320×568, landscape) se reduce lo justo por la altura de la región, sin recortes ni solapes.
- **Verificación**: `node --check` ✅ · suite **123/123** (guardarraíl de medallas incluido) ✅ · eslint ✅ (0 errores, 2 warnings preexistentes) · `balance-sim` 40 runs ✅ (supervivencia skilled oleada 18; guardarraíl verde) · **navegador** (`?dev`): BUG-1 — con `_specialRoom()=0` en oleada 12, `bossEvent`→encuentro con 3 anclas (antes: fallback), banner «La Corriente/Nubarrón · Nv. II · ◆◆◆», lock 1197ms, boss card `animationDuration:2.6s`, ciclo golpe→fase2→derrota→beat+bendición; BUG-3 — **matriz de viewports** (320×568, 360×640, 375×667, 414×896, 1280×720 desktop): tablero **cuadrado** (cellSquare ✓, 64 celdas 8×8), **88vw fijo** en 640/667/896, sin `topOverlap`/`botOverlap`/`overflow`, `booster` siempre dentro del viewport; `FX.boardRect` y `cellXY(0)` mapean correctamente (input OK); **cero errores de consola** ✅. Triple bump 2.6.21→**2.6.23** (BUG-1/2 en 2.6.22; rediseño responsive del layout SV en 2.6.23). Sin claves i18n nuevas. *(Landscape de móvil = degradación aceptable: tablero pequeño pero sin solape, no está pensado para apaisado.)*
...
- **JF-20..24 · Los Cinco Señores completos**: fase 2 con twist propio por jefe — Nubarrón: lluvia deja **1 roca** en el centro de la zona marcada · La Corriente: anillo completo (ya en α) · Boreal: **clúster 2×2** compacto (busca bloques de 4 iconos contiguos; fallback disperso) · **El Cerrajero: la JAULA** (`_cageSteal`: roba 1 potenciador del inventario del intento, lo planta como tile `cage` con `loot`; romperla por adyacencia lo devuelve; sin stock de run cae al cierre normal; `bossatk_lockdown_2` renombrada «Jaula»/«Cage») · Tectónico: terremoto total + **2 rocas** (respetando topes). **Efectos-firma de derrota** (`_defeatEffect`): el cielo escampa (limpia ~12%), la marea se retira (vacía filas exteriores), deshielo total +25 de suministro, jaulas devuelven exactamente lo robado —o +25 de suministro si están vacías—, y el Tectónico **ordena el mundo** (`_clusterGift`: agrupa hasta 8 iconos del tipo más común hacia el centro con FLIP — regalo de combo por intercambios puros).
- **Botín y Ronda maestra (§3.8)**: derrota paga `8 + 4×nivel` monedas (flyer de monedas); flawless (sin perder vida NI gastar potenciador en el encuentro) → **+1 vida** o **+50 de suministro** al tope — homenaje al Master Round de Gungeon.
